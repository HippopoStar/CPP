...
void			Bureaucrat::executeForm(Form const &f) const
{
	...
	{
		std::cout << "Bureaucrat <" << (*this)._name << "> can't execute form <" << f.getName() << "> because it isn't signed yet" <<std::endl;
	}
	else
	{
		try
		{
			f.execute(*this);
			std::cout << "Bureaucrat <" << (*this)._name << "> executes form <" << f.getName() << ">" << std::endl;
		}
